#!/bin/bash
# Test script for pg_read_query tool
set -e
...
echo "=== Testing pg_read_query ==="

RESPONSE=$( ( echo '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"pg_read_query","arguments":{"query":"SELECT 1 AS one"}},"id":4}'; sleep 2 ) | "$SCRIPT_DIR/../bin/postgres-server" --dsn "$PG_DSN" 2>&1 )
...
echo ""
echo "=== pg_read_query test PASSED ==="
